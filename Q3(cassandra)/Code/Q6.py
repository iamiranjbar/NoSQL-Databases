...
for _, row in tqdm(dataset.iterrows(), total=dataset.shape[0]):
    ...
    insert_query = "INSERT INTO Q6(title_album, track_id, title_track, year_released, month_released, day_released," + \
...
start_time = time.time()
rows = session.execute('SELECT COUNT(*) AS song_count FROM Q6 WHERE year_released=2008 AND genre = \'Folk\' AND month_released<7')
print("Query time: {} s".format(time.time() - start_time))
print("Results:")
for row in rows:
    print(row.song_count)
